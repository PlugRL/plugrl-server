...
class DPPOPolicyConfig(BaseDPPOPolicyConfig):
    ...
    min_sampling_denoising_std: float = 0.

@register_policy(UID)
class DPPOPolicy(BaseDPPOPolicy):
    config: DPPOPolicyConfig
    obskeys: list[str]
    model: dppo.diffusion.DiffusionModel
    ...
    def __init__(self, config: DPPOPolicyConfig):
        ...
        self.model: dppo.diffusion.DiffusionModel = hydra.utils.instantiate(cfg.model)
        ...
        self.action_horizon = self.model.horizon_steps
        self.num_denoising_steps = self.model.denoising_steps
    ...
    def _get_timesteps(self) -> torch.Tensor:
        timesteps = list(reversed(range(self.model.denoising_steps)))
    ...
    def prepare_observation(self, _obs: dict) -> torch.Tensor | tensordict.TensorDict:
        state = _obs["states"]
        logger.debug(f"Preparing observation with state keys: {list(self.low_dim_keys)}")
    ...
    def _postprocess_action(self, action: torch.Tensor) -> np.ndarray:
        if self.model.final_action_clip_value is not None:
            action = torch.clamp(action, -self.model.final_action_clip_value, self.model.final_action_clip_value)
        logger.debug(f"Postprocessing action with shape {action.shape} on device {action.device}")
        action_numpy = action.cpu().numpy()
        logger.debug(f"Action numpy shape: {action_numpy.shape}, min: {action_numpy.min()}, max: {action_numpy.max()}")
    ...
    def _denoising_step(
        ...
        mean_logvar: Tuple[torch.Tensor, torch.Tensor] = self.model.p_mean_var(
            x=x, t=t, cond=cond,
        )
        mean, logvar = mean_logvar
        std = torch.clip(torch.exp(0.5 * logvar), min=self.config.min_sampling_denoising_std)

        dist = torch.distributions.Normal(mean, std)

        if x_next is None:
            noise = torch.randn_like(x).clamp_(
                -self.model.randn_clip_value, self.model.randn_clip_value
            )
