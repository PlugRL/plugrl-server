...
from vlarl_launcher.common.checkpoint_manager import Checkpoint, load_checkpoint_from_path
...
class Evaluation(BaseAlgorithm):
    ...
    def __init__(self, config: EvalConfig, policy: BasePolicy):
        ...
        if config.policy_checkpoint_path is not None:
            logger.info(f"Loading policy checkpoint from {config.policy_checkpoint_path}")
    ...
    def get_recorded_episode_stats(self) -> dict:
        ...
        if len(self._record_episode_stats) > 0:
            ...
            for key in keys:
                avg_stats[key] = np.mean([ep_stats[key] for ep_stats in self._record_episode_stats])
        return avg_stats