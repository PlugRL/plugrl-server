...
class DDPWorker:
    ...
    def __init__(
        ...
        assert isinstance(algo, DDPAlgorithm), (
            f"Algorithm {algo_spec.algo_uid} is not a DDPAlgorithm."
        )
        self.algo = algo
        ...
        policy_ddp = torch.nn.parallel.DistributedDataParallel(
            self.algo.policy, device_ids=[self.device], output_device=self.device
        )
    ...
    def load_buffer_data(
        self, global_step: int, meta_info: dict, buffer_data_dict: dict
    ) -> None:
...
class LearnerActor:
    def __init__(
        ...
        *,
        ...
        logger.info(
            f"LearnerActor master address: {self.master_addr}, port: {self.master_port}"
        )
    ...
    def learn(
        self, global_step: int, meta_info: dict, buffer_data_dict
    ) -> tuple[Checkpoint, int, dict]:
        logger.info("Starting DDP training orchestration.")

        load_refs = [
            worker.load_buffer_data.remote(global_step, meta_info, buffer_data_dict)
        ...
        logger.info(
            f"Training cycle completed. Global Step: {global_step}. Master Policy updated."
        )
