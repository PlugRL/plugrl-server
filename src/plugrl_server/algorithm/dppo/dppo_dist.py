...
class DPPOAlgoDistributed(_dppo.DPPOAlgorithm, DDPAlgorithm):
    ...
    def __init__(
        self,
        config: DPPOAlgoDistributedConfig,
        policy: _dppo.BasePolicyGradientDiffusionPolicy,
    ):
    ...
    def activate_ddp(
        self, ddp_policy: torch.nn.parallel.DistributedDataParallel
    ) -> None:
    ...
    def get_server_data(self) -> tuple[int, dict, dict]:
        return (
            self.global_step,
            dict(_episode_stats=self._episode_stats),
            self.rollout_buffer.as_dict(),
        )
