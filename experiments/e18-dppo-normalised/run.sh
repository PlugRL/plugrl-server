#!/usr/bin/env bash
# E18: E17's design, rerun on code that normalises the policy's observations.
#
# E17 ran DPPO on fpo-policy with obs_stats_count at 0.0 for all hundred
# iterations - the identity, on inputs with a 59x spread in scale - because
# maintaining those statistics lived in FPOAlgorithm alone. PR #50 fixes it.
# Nothing else about the design changes: same policy, environment, buffer,
# step budget, seeds and concurrency, and still from a random initialisation.
#
...
# The whole of E18 is that this fix is in the code being run. Without it the
# experiment is E17 again, so find out now rather than from the checkpoints.
"$SERVER_PY" -c "
import sys
from plugrl_server.algorithm.dppo.dppo import DPPOAlgorithm
sys.exit(0 if hasattr(DPPOAlgorithm, '_update_policy_obs_stats') else 1)
" 2>/dev/null || {
  echo "error: $SERVER_PY's DPPO does not maintain observation statistics." >&2
  echo "       This needs the tree from PR #50." >&2
  exit 1; }
