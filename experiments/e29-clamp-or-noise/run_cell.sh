#!/usr/bin/env bash
# One arm of E29 - E24's run_cell.sh with one addition: EXTRA_ALGO_ARGS, word
# split and appended to the algorithm's flags, which is all E29's arms differ
# in. E24's description follows.
#
...
if [ -n "${EXTRA_ALGO_ARGS:-}" ]; then
  read -r -a EXTRA <<< "$EXTRA_ALGO_ARGS"
  ALGO_ARGS+=("${EXTRA[@]}")
fi
...
echo "extra:   ${EXTRA_ALGO_ARGS:-none}"
echo "out:     $OUT"
date '+start %F %T'
