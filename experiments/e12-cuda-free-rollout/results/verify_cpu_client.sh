#!/usr/bin/env bash
# Does the CUDA-free env client behave exactly like the one E11 ran?
#
#   bash verify_cpu_client.sh CLIENT_PYTHON CELL PORT
#
# Same suite, same task, same seed, initial states in order: the run is
# deterministic, so a client that behaves identically must return the same
# successes and the same step count. Anything else means the install changed
# the environment, not just its size.
#
# The server is unchanged and still runs on the GPU. Only the client differs.
set -uo pipefail
...
report_strays() {
  ...
  # Only real python processes count. The venv path appears in this script's
  # own command line too, and pgrep -f would otherwise match the shell running
  # it and refuse to start against itself.
  pids=$(pgrep -f "$pattern" | while read -r p; do
           [ "$(ps -o comm= -p "$p" 2>/dev/null)" = python ] && echo "$p"
         done | paste -sd, -)
  if [ -n "$pids" ]; then
    log "$what: $(echo "$pids" | tr ',' '\n' | wc -l) alive"
    ps -o pid,etimes,args -p "$pids" | cut -c1-150
    return 1
  fi
  return 0
}

log "client python: $CPY"
"$CPY" -c "import torch; print('torch', torch.__version__, 'cuda', torch.cuda.is_available())" 2>&1 | tail -1
...
(
  cd "$OUT" && exec setsid env \
    ...
    timeout 2400 "$CPY" -m plugrl_env_client.cli libero-v1 \
...
log "=== summary ==="
find "$OUT" -name summary.json -exec cat {} \; 2>/dev/null | head -25
log "VERIFY_DONE $CELL"
