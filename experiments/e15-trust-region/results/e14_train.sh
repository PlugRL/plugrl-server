...
log "cell=$CELL iterations=$ITERATIONS master_device=$MASTER_DEVICE lr=${LR:-1e-5} updates=${UPDATES:-4} clip=${CLIP_EPS:-0.05} warmup=${WARMUP:-0} srv_gpus=${SRV_GPUS:-0,1} cli_gpu=${CLI_GPU:-2} buffer=$BUFFER_SIZE batch=$BATCH_SIZE"
...
(
  cd "$R/plugrl-server" && exec setsid env \
    ...
    "$SPY" -m plugrl_server.cli pi0-policy default fpo default \
      ...
      --algo.critic-warmup-iterations ${WARMUP:-0} \
