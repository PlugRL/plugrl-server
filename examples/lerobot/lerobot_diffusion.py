...
from plugrl_server.policy.base_policy_gradient_diffusion_policy import BasePolicyGradientDiffusionPolicyConfig, BasePolicyGradientDiffusionPolicy
...
class LeRobotDiffusionPolicy(BasePolicyGradientDiffusionPolicy):    
    def __init__(self, config: LeRobotDiffusionPolicyConfig):
        ...
        assert isinstance(policy_config, DiffusionConfig), f"Expected DiffusionConfig, got {type(policy_config)}"
        policy = DiffusionPolicy.from_pretrained(config.pretrained_path, config=policy_config)
        ...
        self.critic = ValueHead(
            input_dim=self.actor.diffusion.unet.up_modules[0][0].cond_encoder[1].in_features - policy_config.diffusion_step_embed_dim
        )
    ...
    def prepare_observation(self, _obs: dict) -> torch.Tensor | TensorDict:
        ...
        for i, key in enumerate(image_map):
            obs[image_map[key]] = torch.tensor(
                np.stack(
                    [_obs["images"][f"{key}_{j}"] for j in range(n_obs_steps)], axis=1
                ),
                device=self.device,
                dtype=torch.float32,
            ).permute(0, 1, 4, 2, 3) / 255.0  # B, T, C, H, W
        obs_normalized = self.actor.normalize_inputs(obs)
        obs_normalized[OBS_IMAGES] = torch.stack([obs_normalized[key] for key in self.actor.config.image_features], dim=-4)
    ...
    def preprocess_observation(self, obs: TensorDict) -> Any:
        processed_cond = self.actor.diffusion._prepare_global_conditioning(obs.to(self.device))
        return processed_cond
    
    def previous_timestep(self, noise_scheduler: DDPMScheduler, t: torch.Tensor) -> torch.Tensor:
        if noise_scheduler.custom_timesteps or noise_scheduler.num_inference_steps:
            ...
            safe_indices = torch.clamp(next_indices, max=len(noise_scheduler.timesteps) - 1)
    ...
    def _get_variance(self, noise_scheduler: DDPMScheduler, t: torch.Tensor, predicted_variance=None, variance_type=None):
        prev_t = self.previous_timestep(noise_scheduler, t.cpu())

        alpha_prod_t = noise_scheduler.alphas_cumprod[t.cpu()][:, None, None]
        alpha_prod_t_prev = torch.where(prev_t >= 0, noise_scheduler.alphas_cumprod[prev_t.cpu()], noise_scheduler.one)[:, None, None]
    ...
    def _denoising_step(
        ...
        *,
        ...
        b = x.shape[0]
        assert t.shape == (b,)
        assert x.shape == (b, self.action_horizon, self.action_dim), f"Expected x shape ({b}, {self.action_horizon}, {self.action_dim}), got {x.shape}"
        ...
        if b_cond * self.num_denoising_steps == b:
            global_cond = torch.repeat_interleave(processed_cond, self.num_denoising_steps, dim=0)
        ...
        alpha_prod_t = noise_scheduler.alphas_cumprod[t.cpu()][:, None, None]
        alpha_prod_t_prev = torch.where(prev_t >= 0, noise_scheduler.alphas_cumprod[prev_t], noise_scheduler.one)[:, None, None]
        ...
        if noise_scheduler.variance_type == "fixed_small_log":
            variance = self._get_variance(noise_scheduler, t, predicted_variance=predicted_variance)
        elif noise_scheduler.variance_type == "learned_range":
            variance = self._get_variance(noise_scheduler, t, predicted_variance=predicted_variance)
        else:
            variance = (self._get_variance(noise_scheduler, t, predicted_variance=predicted_variance) ** 0.5)
    ...
    def _postprocess_action(self, action: torch.Tensor, obs: torch.Tensor | TensorDict) -> Any:
